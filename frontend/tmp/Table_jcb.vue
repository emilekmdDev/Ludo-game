<template>
  ...
  <div v-show="!showparty">
    ...
    <form @submit.prevent="start">
      <Player v-for="(player,index) in players" :key="index" @del="deleteplayer(player.playername,player.type)" :players="players" :index="index" :colors="colors" :hide='true'></Player>
      <button type="submit">start a party width boot</button>
    </form>
  </div>
<transition>
  <div v-show="showparty">
...
export default {
  ...
  setup(){
    ...
    let players=ref([
      ...
      {playername:'',color:'p2color',turn:false,id:0,pions:null,road:null,pions_t:0,winner:false,type:'bot'},
      {playername:'',color:'p3color',turn:false,id:0,pions:null,road:null,pions_t:0,winner:false,type:'bot'},
      {playername:'',color:'p4color',turn:false,id:0,pions:null,road:null,pions_t:0,winner:false,type:'bot'},
    ])
        ...
        const interval=setInterval(() => {
          ...
          //cas ou aucun pion n'est deplacable on passeras directement au jouerur suivant
              if (movable(dicevalue,players.value[i].pions)==false) { 
                  ...
                  if (nextplayersisbot(players.value)) setTimeout(() => {play()}, 1500);
    ...
    function movable_p(player,status,dice) {
      switch (status) {
        case 'activate':          
            for (let i = 0; i < player.pions.length; i++){
            //cas ou un pion est sortable de la maison
              if (!player.pions[i].finish && dice==6 && player.road.length>=player.pions[i].current_position+dice ||
    ...
    function deleteplayer(removename,type){
      
      /////////empeche le fait de faire une partie constituer uniquement de boot
      let n=0
      for (let i = 0; i < players.length; i++) {
    ...
    function start(){
      ...
      for (let i = 0; i < 4; i++) {
        if (players.value[i].type=='bot') {
          colors.value[i].taked=true
          players.value[i].playername=names[Math.floor(Math.random() * 10)]
        }
      }
