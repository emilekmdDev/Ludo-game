<template>
    ...
    <div v-show="!showparty">
        ...
        <div v-if="!start">
            ...
            <button :disabled="members<2 || members>4">start</button>
            <h3>{{ error }}</h3>
            <Room :members="members" :users="users"></Room>
        </div>
    ...
    <div class="container">
      <div class="c1">
        <div class="p1color home">
          ...
          <div v-for="i in players.length" :key="i">
            ...
            <div class="subcontainer" v-if="players[i-1].color=='p1color'" id="1">
              {{ players[i-1].playername}}
              {{ players[i-1].turn }}
              <div class="pion" :class="{waiting:pionsp1[0].canmove && !pionsp1[0].finish}" ref="varpion1p1" @click="select(pionsp1[0],pionsp1)"></div>
              <div class="pion" :class="{waiting:pionsp1[1].canmove && !pionsp1[1].finish}" ref="varpion2p1" @click="select(pionsp1[1],pionsp1)"></div>
              <div class="pion" :class="{waiting:pionsp1[2].canmove && !pionsp1[2].finish}" ref="varpion3p1" @click="select(pionsp1[2],pionsp1)"></div>
              <div class="pion" :class="{waiting:pionsp1[3].canmove && !pionsp1[3].finish}" ref="varpion4p1" @click="select(pionsp1[3],pionsp1)"></div>
            </div>
          </div>
        ...
        <div class="p4color home" >
          <!-- gere l'affichage des pions en fonctions des couleurs selectionner -->
          <div v-for="i in players.length" :key="i">
            <div class="subcontainer" v-if="players[i-1].color=='p4color'" id="4">
              {{ players[i-1].playername }}
              {{ players[i-1].turn }}
              <div class="pion" :class="{waiting:pionsp4[0].canmove && !pionsp4[0].finish}" ref="varpion1p4" @click="select(pionsp4[0],pionsp4)"></div>
              <div class="pion" :class="{waiting:pionsp4[1].canmove && !pionsp4[1].finish}" ref="varpion2p4" @click="select(pionsp4[1],pionsp4)"></div>
              <div class="pion" :class="{waiting:pionsp4[2].canmove && !pionsp4[2].finish}" ref="varpion3p4" @click="select(pionsp4[2],pionsp4)"></div>
              <div class="pion" :class="{waiting:pionsp4[3].canmove && !pionsp4[3].finish}" ref="varpion4p4" @click="select(pionsp4[3],pionsp4)"></div>
            </div>
          </div>
      ...
      <div class="c3">
        <div class="p2color home">
          <!-- gere l'affichage des pions en fonctions des couleurs selectionner -->
          <div v-for="i in players.length" :key="i">
            <div class="subcontainer" v-if="players[i-1].color=='p2color'" id="2">
              {{ players[i-1].playername }}
              {{players[i-1].turn}}
              <div class="pion" :class="{waiting:pionsp2[0].canmove && !pionsp2[0].finish}" ref="varpion1p2" @click="select(pionsp2[0],pionsp2)"></div>
              <div class="pion" :class="{waiting:pionsp2[1].canmove && !pionsp2[1].finish}" ref="varpion2p2" @click="select(pionsp2[1],pionsp2)"></div>
              <div class="pion" :class="{waiting:pionsp2[2].canmove && !pionsp2[2].finish}" ref="varpion3p2" @click="select(pionsp2[2],pionsp2)"></div>
              <div class="pion" :class="{waiting:pionsp2[3].canmove && !pionsp2[3].finish}" ref="varpion4p2" @click="select(pionsp2[3],pionsp2)"></div>
            </div>
          </div>
        </div>
        <div class="block1">
        ...
        <div class="p3color home">
          <!-- gere l'affichage des pions en fonctions des couleurs selectionner -->
          <div v-for="i in players.length" :key="i">
            <div class="subcontainer" v-if="players[i-1].color=='p3color'" id="3">
              {{ players[i-1].playername }}
              {{ players[i-1].turn }}
              <div class="pion" :class="{waiting:pionsp3[0].canmove && !pionsp3[0].finish}" ref="varpion1p3" @click="select(pionsp3[0],pionsp3)"></div>
              <div class="pion" :class="{waiting:pionsp3[1].canmove && !pionsp3[1].finish}" ref="varpion2p3" @click="select(pionsp3[1],pionsp3)"></div>
              <div class="pion" :class="{waiting:pionsp3[2].canmove && !pionsp3[2].finish}" ref="varpion3p3" @click="select(pionsp3[2],pionsp3)"></div>
              <div class="pion" :class="{waiting:pionsp3[3].canmove && !pionsp3[3].finish}" ref="varpion4p3" @click="select(pionsp3[3],pionsp3)"></div>
            </div>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    setup(){
      ...
      let players=ref([
        {playername:'player 1',color:'p1color',turn:true,id:0,pions:null,road:null,pions_t:0,winner:false,type:'human'},
        {playername:'player 2',color:'p2color',turn:false,id:0,pions:null,road:null,pions_t:0,winner:false,type:'human'}
      ])
      
      let partie=new Partie()
      
      /////////////////////////////////////////////// continet tout les couleur dispo et permet de savoir ques couleur sont disponible lor de la selection des joueur
      let colors=ref([{color:'p1color',taked:true},{color:'p2color',taked:true},{color:'p3color',taked:false},{color:'p4color',taked:false}])
      ...
      function move(pion,valeur,color) {
        ...
        if (index<road.length) {
          ...
          // let position2 = pion.domvar.getBoundingClientRect()
          // let x2 = position2.left;
          // let y2 = position2.top;
  
          // pion.domvar.style.transform=`translate(${(x-x2)}px, ${y-y2}px)`
          // console.log(x-x2,y-y2,target);
      ...
      function backhome(pion,player){
        ...
        switch (player.color) {
            ...
            case 'p1color':
              tmp = document.getElementById('1')
              tmp.appendChild(pion.domvar)
            break;
          
            case 'p2color':
              tmp = document.getElementById('2')
              tmp.appendChild(pion.domvar)
            break;
            
            case 'p3color':
              tmp = document.getElementById('3')
              tmp.appendChild(pion.domvar)
            break;
            
            case 'p4color':
              tmp = document.getElementById('4')
              tmp.appendChild(pion.domvar)
            break;
      ...
      function movable(dice,pions) {
        for (let i = 0; i < pions.length; i++){
          ...
          if (!pions[i].finish && dice==6){
            return true
          }
          
          // cas ou un pion est en cours de route
          if (!pions[i].finish && pions[i].current_position!=-1){
            return true  
          }
        }
        return false
      ...
      function movable_p(player,status,dice) {
        switch (status) {
          case 'activate':          
              for (let i = 0; i < player.pions.length; i++){
              //cas ou un pion est sortable de la maison
                if (!player.pions[i].finish && dice==6 && player.road.length>=player.pions[i].current_position+dice || !player.pions[i].finish && player.pions[i].current_position!=-1 && player.road.length>=player.pions[i].current_position+dice){
      ...
      function addplayer(name){
        if (players.value.length<4 && name!='') {
          ...
          players.value.push({playername:name,color:color_.color,turn:false,id:0,pions:null,road:null,pions_t:0,winner:false,type:'human'})
          // colors.value[players.value.length-1].taked=true
          color_.taked=true
        }
        playername.value=''
      }
